...
import ResellerTransactionCreated from "App/Notifications/ResellerTransactionCreated";
...
export default class ResellerTransaction extends BaseModel {
  ...
  public static async notifyReseller(self: ResellerTransaction){
    ...
    await self.load('transaction', p=>p.preload("customer"));
    await TrackingListeners
      .onResellerTransactionCreated(self.transaction)
    const { reseller : { user } } = self;

    user.notify(new ResellerTransactionCreated(self.transaction));
  }
